                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <h4 className="text-lg md:text-xl font-bold text-white uppercase truncate">{acc.username || acc.email}</h4>
                          <CopyButton text={acc.username || acc.email} />
                        </div>
                        <div className="flex items-center gap-2">
                          <p className="text-[10px] text-slate-500 font-bold tracking-tight truncate">{acc.email}</p>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <select value={value} onChange={e => onChange(e.target.value)} required className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-accent/50 transition-all appearance-none cursor-pointer">
        ...
        {accounts.map(acc => <option key={acc.id} value={acc.id} className="bg-slate-900">{acc.username || acc.email}</option>)}
      </select>
    </div>
  )
}
